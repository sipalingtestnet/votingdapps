...
const ChatButton: React.FC<{ poll: PollStruct; group: any }> = ({ poll, group }) => {
  ...
  return (
    <Menu as="div" className="inline-block text-left mx-auto fixed right-5 bottom-[80px]">
      <Menu.Button
        className="bg-[#EF6C00] hover:bg-orange-700 text-white font-bold
      ...
      <Menu.Items
        className="absolute right-0 bottom-14 mt-2 w-56 origin-top-right
          divide-y divide-neutral-100 rounded-md bg-white shadow-lg shadow-black
          ...
          <>
              ...
              {({ active }) => (
                <button
                  className={`flex justify-start items-center space-x-1 ${
                    active ? 'bg-neutral-200 text-black' : 'text-red-500'
            ...
            <Menu.Item>
              {({ active }) => (
                <button
                  className={`flex justify-start items-center space-x-1 ${
                    active ? 'bg-neutral-200 text-black' : 'text-neutral-900'
                  } group flex w-full items-center rounded-md px-2 py-2 text-sm`}
                  onClick={handleLogin}
                >
                  <FiLogIn size={17} />
